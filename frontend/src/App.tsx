...
export default function App() {
  ...
  return (
    <div className="flex h-[100dvh] max-h-[100dvh] w-screen flex-col overflow-hidden bg-[#080A0D] text-[#F5F7FA]">
      {/* TopBar (Header: 64px mobile / 76px desktop) */}
      <TopBar
        sidebarOpen={sidebarOpen}
        onToggleSidebar={() => setSidebarOpen((v) => !v)}
      />

      {/* Main 3-Column Workspace (Fixed 100dvh viewport) */}
      <main className="grid min-h-0 flex-1 grid-cols-1 gap-2.5 p-2.5 sm:p-3.5 sm:gap-3.5 lg:grid-cols-[300px_minmax(0,1fr)] xl:grid-cols-[300px_minmax(0,1fr)_320px] overflow-hidden">
          ...
          {sidebarOpen && (
            <>
              ...
              <motion.div
                ...
                className="fixed inset-y-0 left-0 z-50 w-[min(280px,85vw)] p-2.5 lg:hidden"
              >
                <Sidebar />
              </motion.div>
            </>
        ...
        <section className="flex min-h-0 flex-1 flex-col gap-2.5 sm:gap-3.5 overflow-hidden h-full">
          {activeTab === 'documents' ? (
            ...
            <>
              ...
              <div className="card-premium relative flex min-h-0 flex-1 flex-col rounded-2xl overflow-hidden bg-[#11151C]/90 border border-white/[0.07]">
                ...
                <div className="absolute inset-0 z-0">
                  ...
                  <div className="pointer-events-none absolute inset-x-0 top-0 h-14 sm:h-16 bg-gradient-to-b from-[#080A0D]/70 to-transparent" />
                  <div className="pointer-events-none absolute inset-x-0 bottom-0 h-24 sm:h-28 bg-gradient-to-t from-[#0B0E13]/90 via-[#0B0E13]/60 to-transparent" />
                </div>

                {/* Top Stage Header Overlay */}
                <div className="relative z-20 flex items-start justify-between p-3 sm:p-4 pointer-events-none">
                  <div>
                    <div className="flex items-center gap-1.5">
                      <span className="h-1.5 w-1.5 rounded-full bg-amber-primary animate-pulse" />
                      <p className="font-mono text-[9.5px] sm:text-[10px] font-bold uppercase tracking-[0.2em] text-amber-primary">
                        Document Intelligence Stage
                      </p>
                    </div>
                    <p
                      className={cn(
                        'mt-0.5 font-mono text-[10px] sm:text-[11px]',
                  ...
                  >
                    {stageExpanded ? <Minimize2 className="h-3 w-3 sm:h-3.5 sm:w-3.5" /> : <Maximize2 className="h-3 w-3 sm:h-3.5 sm:w-3.5" />}
                  </button>
                </div>
                ...
                <div className="relative z-10 flex min-h-0 flex-1 flex-col overflow-hidden">
                  {documents.length === 0 && messages.length === 0 ? (
                    ...
                    <div className="flex min-h-0 flex-1 flex-col p-2.5 sm:p-4 overflow-hidden">
                      <ChatFeed />
                    </div>
                  )}
                </div>
        ...
        {/* Right Column: Knowledge Base Metrics, System Status & Quick Tips (Desktop xl screens only) */}
